...
| NumPy sampler in `test_mpi/sampler.py` | `runtime/speculative/sampler.py` (torch, CPU) |
| `test_mpi/protocol.py` wire helpers | Move to `runtime/speculative/mpi_protocol.py` or into `mpi_coordinator.py` |
| `test_mpi/main.py` rank loops | `runtime/speculative/mpi_coordinator.py` |
...
srun --partition=cpu --nodes=1 --ntasks=1 --cpus-per-task=4 \
  bash -lc 'module load course/cme213/nvhpc/24.1; mpirun --oversubscribe -np 2 python -m test_mpi.main'
```
...
srun --partition=gpu-turing --gres=gpu:2 --ntasks=1 --cpus-per-task=4 \
  mpirun --oversubscribe -np 2 python -m test_mpi.main
```
...
2. **torch + mpirun on login node** can segfault even without CUDA. Production target rank will import torch (GPU); that is fine on compute nodes with GPUs. Mock uses NumPy-only `test_mpi/sampler.py` for login-node testing.
...
PYTHONNOUSERSITE=1 python -m unittest test_mpi.tests.test_mock_step -v

# Local 2-rank (login node; NVHPC module loaded)
bash test_mpi/run_mpi.sh
bash test_mpi/run_mpi.sh --steps 20 --gamma 3
...
`bash test_mpi/run_mpi.sh --slurm-gpu` automatically passes `--require-gpu`. Each MPI rank:
...
```bash
bash test_mpi/run_mpi.sh --slurm-gpu --steps 3 --gamma 2
# or: mpirun -np 2 python -m test_mpi.main --require-gpu ...
```
...
### 1. `runtime/speculative/mpi_protocol.py` (from `test_mpi/protocol.py`)
...
### 2. `runtime/speculative/mpi_coordinator.py` (from `test_mpi/main.py`)
...
### 3. `slurm/run_speculative.sh` (from `test_mpi/run_mpi.sh`)
...
- `test_mpi/sampler.py` — runtime already has `runtime/speculative/sampler.py`

---

## Mapping to Phase 8 plan

| Phase | Status | test_mpi coverage |
|-------|--------|-------------------|
