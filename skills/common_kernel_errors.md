...
Field notes from getting the first CUDA extension (`src/kernels/embedding/`) to
...
| `course/cme213/nvhpc/24.1` | Course MPI / nvc++ for later assignments. *Can stay loaded* — `src/kernels/*/jit.py` sanitizes its env-var pollution at build time. | `mpicc`, `nvc++`, NVHPC libs |
...
before invoking the build. Implemented in `src/kernels/embedding/jit.py:
...
srun --partition=gpu-turing --gres=gpu:1 python src/kernels/embedding/test.py
