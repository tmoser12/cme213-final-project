#!/bin/bash
#
# Unified benchmark + profile driver for the swiglu extension.
#
# Usage:
...
KERNEL_DIR=$(basename "$_KERNEL_PARENT")            # "swiglu"
KERNEL_REGEX="swiglu_act_kernel"                    # matches kernel.cu
MODULE="kernel_dev.target.kernels.${KERNEL_DIR}.benchmark"
...
case "$_KERNEL_PARENT" in
    */kernel_dev/draft/*)  MODEL=draft ;;
    */kernel_dev/target/*) MODEL=target ;;
    *)              MODEL=unknown ;;
esac
case "$KERNEL_DIR" in
    swiglu)    GROUP=swiglu ;;
