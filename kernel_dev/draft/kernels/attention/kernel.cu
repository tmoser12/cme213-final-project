...
// blocks (14 for the 0.5B draft) -> <20% of the RTX 6000's 72 SMs, and its
...
//
// num_splits is chosen on the host (choose_num_splits) so total blocks fill the
// SMs while each split keeps >= MIN_SPLIT_LEN rows of work; num_splits==1 falls
// back to decode_attn_kernel above (short context pays no combine overhead).
// ===========================================================================
    ...
    const __half* __restrict__ sin)
{
    ...
    // This split owns cache rows [kv_start, kv_end). kv_start is a multiple of
    // KV_TILE (split_len is rounded up to KV_TILE host-side), so tiles align.
    ...
    // --- Load Q + init running softmax state -------------------------------
    for (int i = tid; i < Q_TOKENS * D; i += NUM_THREADS) {
        const int r = i / D, d = i % D;
        q_sh[r][d] = q[q_slab_base + r * D + d];
        acc[r][d]  = 0.0f;
    ...
    for (int tile0 = kv_start; tile0 < kv_end; tile0 += KV_TILE) {
        ...
        // (1) Load tile; rows past this split's kv_end are zero-filled.
        for (int i = tid; i < KV_TILE * D; i += NUM_THREADS) {
            const int kt = i / D, d = i % D;
            const int kpos = tile0 + kt;
        ...
        // (2) Scores. Mask non-causal keys and keys outside this split's range.
        for (int p = tid; p < Q_TOKENS * KV_TILE; p += NUM_THREADS) {
            const int r    = p / KV_TILE;
            const int kt   = p % KV_TILE;
            const int kpos = tile0 + kt;
        ...
        // (3) Online-softmax update (one thread per query row).
        if (tid < Q_TOKENS) {
            const int r = tid;
            float tile_max = -INFINITY;
            #pragma unroll
    ...
    // --- Write UN-normalized partials (acc is sum exp(s-m)*v, NOT /l) -------
    const int pslot = (batch * h_q + head) * num_splits + split;   // (b,head,split)
    for (int i = tid; i < Q_TOKENS * D; i += NUM_THREADS) {
        const int r = i / D, d = i % D;
...
static torch::Tensor launch_decode_attn(const torch::Tensor& q,
    ...
    const int cl = static_cast<int>(cur_len);
    ...
    int num_splits = choose_num_splits(cl, h_q, B);
    ...
    if (num_splits <= 1) {
        ...
        decode_attn_kernel<Q_TOKENS, D, KV_TILE, NUM_THREADS><<<grid, block>>>(
            ...
            static_cast<float>(softmax_scale),
            h_q, h_kv, max_seq, cl, cos_ptr, sin_ptr);
        C10_CUDA_KERNEL_LAUNCH_CHECK();
        return o;
    }

    // --- Split-KV path: partition the KV axis across grid.x, then combine ---
    // Round split_len up to a KV_TILE multiple so each split starts tile-aligned,
    // then recompute num_splits to exactly cover [0, cur_len).
    int split_len = (cl + num_splits - 1) / num_splits;
    split_len = ((split_len + KV_TILE - 1) / KV_TILE) * KV_TILE;
    num_splits = (cl + split_len - 1) / split_len;
    ...
    dim3 block(NUM_THREADS);
    decode_attn_split_kernel<Q_TOKENS, D, KV_TILE, NUM_THREADS><<<grid, block>>>(
        q_ptr, ck_ptr, cv_ptr,
        partial_o.data_ptr<float>(), partial_m.data_ptr<float>(), partial_l.data_ptr<float>(),
        static_cast<float>(softmax_scale),
        h_q, h_kv, max_seq, cl, num_splits, split_len, cos_ptr, sin_ptr);
    C10_CUDA_KERNEL_LAUNCH_CHECK();

    dim3 cgrid(h_q, B);
    dim3 cblock(D);   // one lane per output channel
    decode_attn_combine_kernel<Q_TOKENS, D><<<cgrid, cblock>>>(
        partial_o.data_ptr<float>(), partial_m.data_ptr<float>(), partial_l.data_ptr<float>(),
