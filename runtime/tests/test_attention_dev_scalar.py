"""Phase 2 parity: device-scalar attention ops vs host-int ops (must be bit-exact).

The `_dev` variants read `write_pos` / `cur_len` from a 0-d int64 CUDA tensor
instead of a host int, so a CUDA graph can update the position between replays.
Outside a graph they must compute *exactly* the same thing as the host-int path.
This test asserts bit-exact equality at several positions; it does NOT need HF.

Run: bash slurm/run_tests_gpu.sh runtime.tests.test_attention_dev_scalar
"""
...
# 7B attention dims (kernels are templated on head_dim=128).
NH, NKV, D = 28, 4, 128
MAX_SEQ = 96
...
class TestAttentionDevScalar(unittest.TestCase):
    def setUp(self):
        torch.manual_seed(0)

    def _identity_rope(self, S):
        # cos=1, sin=0 -> RoPE is identity; values are irrelevant to parity (both
        # paths get the same tensors) but this keeps the math clean and finite.
    ...
    def test_rope_kv_write_dev_matches_host(self):
        ...
        for write_pos in (0, 1, 30, MAX_SEQ - S):
            ...
            cos, sin = self._identity_rope(S)
            # Two independent caches written by the two code paths; compare.
    ...
    def test_dev_rejects_bad_scalar(self):
        q = _rand(1, NH, 1, D)
        cache_k = _rand(1, NKV, MAX_SEQ, D)
        cache_v = _rand(1, NKV, MAX_SEQ, D)
        # cur_len must be a CUDA int64 0-d scalar; wrong dtype/device should raise.
        with self.assertRaises(Exception):
            decode_attn_forward_dev(q, cache_k, cache_v, torch.tensor(5), SCALE)  # CPU int64
        with self.assertRaises(Exception):
            decode_attn_forward_dev(q, cache_k, cache_v,
                                    torch.tensor(5.0, dtype=torch.float32, device=DEV), SCALE)
