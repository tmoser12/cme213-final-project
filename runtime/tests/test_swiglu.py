"""Tests for SwiGLU AOT kernel integration (Phase 3).
...
from runtime.production_kernels.draft.swiglu import swiglu_forward as draft_swiglu_forward
from runtime.production_kernels.draft.swiglu.ops import EXTENSION_MODULE as DRAFT_EXTENSION_MODULE
...
def _cfg_to_qwen2_config(cfg: RuntimeConfig):
...
class TestSwigluAot(unittest.TestCase):
    ...
    def test_draft_extension_importable(self) -> None:
        ...
        self.assertTrue(hasattr(ext, "swiglu_forward"))
        self.assertNotIn("torch_extensions", ext.__file__)
        self.assertIn("production_kernels/draft/swiglu", ext.__file__.replace("\\", "/"))
...
class TestSwigluGpu(unittest.TestCase):
    def test_forward_matches_hf_7b(self) -> None:
        ...
        hf = Qwen2MLP(_cfg_to_qwen2_config(cfg)).cuda().half()
...
@unittest.skipIf(REQUIRES_GPU, GPU_SKIP)
class TestSwigluDraftGpu(unittest.TestCase):
