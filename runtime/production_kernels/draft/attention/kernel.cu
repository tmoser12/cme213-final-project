...
// into cache_k/cache_v at [..., write_pos:write_pos+S, :].
...
// tensor read on the device. Bounds on write_pos are the caller's responsibility.
...
// tensor read on the device. S (query length) stays a host shape (fixed per graph).
