"""Python host ABI for draft RMSNorm (AOT extension in this directory)."""

from __future__ import annotations

_EXT_MODULE = "runtime.production_kernels.draft.rmsnorm.draft_rmsnorm_ops"
_BUILD_HINT = "Run: bash scripts/build_kernels.sh draft rmsnorm"


def _load_ext():
    try:
        from . import draft_rmsnorm_ops as ext
    ...
    if "production_kernels/draft/rmsnorm" not in path.replace("\\", "/"):
        raise ImportError(
            f"Expected AOT extension under draft/rmsnorm/, got: {path!r}. "
            + _BUILD_HINT
        )
    return ext
