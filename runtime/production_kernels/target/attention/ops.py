"""Python host ABI for target attention (AOT extension in this directory)."""
...
_EXT_MODULE = "runtime.production_kernels.target.attention.target_attention_ops"
_BUILD_HINT = "Run: bash scripts/build_kernels.sh attention"


def _load_ext():
    try:
        from . import target_attention_ops as ext
    ...
    if "production_kernels/target/attention" not in path.replace("\\", "/"):
        raise ImportError(
            f"Expected AOT extension under target/attention/, got: {path!r}. "
            + _BUILD_HINT
        )
    return ext
