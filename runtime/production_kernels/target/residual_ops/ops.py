"""Python host ABI for target residual_ops (AOT extension in this directory)."""

from __future__ import annotations

_EXT_MODULE = "runtime.production_kernels.target.residual_ops.target_residual_ops"
_BUILD_HINT = "Run: bash scripts/build_kernels.sh residual_ops"


def _load_ext():
    try:
        from . import target_residual_ops as ext
    ...
    if "production_kernels/target/residual_ops" not in path.replace("\\", "/"):
        raise ImportError(
            f"Expected AOT extension under target/residual_ops/, got: {path!r}. "
            + _BUILD_HINT
        )
    return ext
