"""Python host ABI for target SwiGLU (AOT extension in this directory)."""

from __future__ import annotations

_EXT_MODULE = "runtime.production_kernels.target.swiglu.target_swiglu_ops"
_BUILD_HINT = "Run: bash scripts/build_kernels.sh swiglu"


def _load_ext():
    try:
        from . import target_swiglu_ops as ext
    ...
    if "production_kernels/target/swiglu" not in path.replace("\\", "/"):
        raise ImportError(
            f"Expected AOT extension under target/swiglu/, got: {path!r}. "
            + _BUILD_HINT
        )
    return ext
