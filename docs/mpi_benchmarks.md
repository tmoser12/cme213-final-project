...
numbers come from the `mpi_prototype/` micro-benchmark of the same payloads.

**Implementation:** `mpi_prototype/benchmark.py`  
**Run:** `bash mpi_prototype/run_mpi.sh --slurm-gpu --benchmark`
...
- Draft logits are sent as a `uint8` view of the fp16 buffer — direct `float16` `Send`/`Recv` segfaults on NVHPC Open MPI (see `mpi_prototype/README.md`).
...
bash mpi_prototype/run_mpi.sh --slurm-gpu --benchmark

# Custom trial count
bash mpi_prototype/run_mpi.sh --slurm-gpu --benchmark --trials 50 --warmup 5

# Skip target H2D after recv (MPI-only on target side)
bash mpi_prototype/run_mpi.sh --slurm-gpu --benchmark --no-h2d-after-recv
```
...
cd "$PROJECT_ROOT"
env PYTHONNOUSERSITE=1 mpirun --oversubscribe -np 2 python -m mpi_prototype.benchmark
```
...
| File | Role |
|------|------|
| `mpi_prototype/benchmark.py` | Benchmark entry point |
| `mpi_prototype/protocol.py` | Wire format + `send_*` / `recv_*` helpers |
| `mpi_prototype/run_mpi.sh` | SLURM launcher (`--benchmark` flag) |
| `mpi_prototype/README.md` | Full MPI prototype docs + port checklist |
