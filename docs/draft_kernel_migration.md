...
`kernel_dev/draft/kernels/` holds the inference kernels for the **Qwen2.5-0.5B draft model**.
...
module paths and every Python import were repointed `kernel_dev.target.kernels.*` →
`kernel_dev.draft.kernels.*`, and cache-clear paths updated to `draft_<dir>_ops`.

## Status

| Kernel | Status | Change summary |
|---|---|---|
| build isolation | ✅ code done | rename `draft_*_ops`, fix `kernel_dev.draft` paths everywhere |
...
1. **decode `NUM_THREADS` at D=64.** Kept at 128. In the PV/acc phase
   ...
   unclear — **profile before changing.** Related: `docs/decode_attention_design.md` split-K +
...
```bash
bash kernel_dev/draft/kernels/rmsnorm/run_benchmark.sh        # fast — validates the draft_*_ops rename
bash kernel_dev/draft/kernels/swiglu/run_benchmark.sh
bash kernel_dev/draft/kernels/residual_ops/run_benchmark.sh
bash kernel_dev/draft/kernels/attention/run_benchmark.sh benchmark_decode_attn   # PRIMARY draft path
bash kernel_dev/draft/kernels/attention/run_benchmark.sh benchmark_rope
bash kernel_dev/draft/kernels/attention/run_benchmark.sh benchmark_kv_write
bash kernel_dev/draft/kernels/attention/run_benchmark.sh benchmark_qkv_proj
bash kernel_dev/draft/kernels/attention/run_benchmark.sh benchmark_o_proj
bash kernel_dev/draft/kernels/attention/run_benchmark.sh benchmark_fused_attn    # prefill
# embedding has no run_benchmark.sh: srun -m kernel_dev.draft.kernels.embedding.benchmark (fix local import first if needed)
```
