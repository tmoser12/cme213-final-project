...
`bash src/kernels/attention/run_benchmark.sh benchmark_decode_attn`.
...
4. **Runtime/wrapper dispatch (optional)** — teach `wrapper.py` / the future
   `src/runtime/` op-dispatch to route small `S` to these ops and large `S` to
   `fused_attn_forward` behind one `attention.run(...)` interface (tmp.md
   Phase 3). The current `wrapper.py` raises `NotImplementedError` on
