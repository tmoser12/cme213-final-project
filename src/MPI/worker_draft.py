...
def run_draft_worker(
    ...
    rank_log(rank, f"Loading draft model from {config.draft_model_path} on {device}")
    load_start = now()
    tokenizer = AutoTokenizer.from_pretrained(config.draft_model_path, trust_remote_code=True)
