...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("fused_attn_forward",  &fused_attn_forward,  "Fused causal SDPA, GQA-aware, prefill (fp16)");
    m.def("decode_attn_forward", &decode_attn_forward, "Decode attention, S==1 (fp16)");
    m.def("small_q_attn_forward",&small_q_attn_forward,"Decode attention, S in [1,8] verify (fp16)");
